...
BuildParameters.Tasks.CreateReleaseNotesTask = Task("Create-Release-Notes")
    .Does(() => RequireTool(BuildParameters.IsDotNetBuild || BuildParameters.PreferDotNetGlobalToolUsage ? ToolSettings.GitReleaseManagerGlobalTool : ToolSettings.GitReleaseManagerTool, () => {
        ...
        GitReleaseManagerCreate(gitReleaseManagerCredentials.Token, BuildParameters.RepositoryOwner, BuildParameters.RepositoryName, settings);
    })
);
...
BuildParameters.Tasks.ExportReleaseNotesTask = Task("Export-Release-Notes")
    ...
    .Does(() => RequireTool(BuildParameters.IsDotNetBuild || BuildParameters.PreferDotNetGlobalToolUsage ? ToolSettings.GitReleaseManagerGlobalTool : ToolSettings.GitReleaseManagerTool, () => {
        ...
        if (BuildParameters.ShouldDownloadMilestoneReleaseNotes)
        {
            ...
            GitReleaseManagerExport(gitReleaseManagerCredentials.Token, BuildParameters.RepositoryOwner, BuildParameters.RepositoryName, BuildParameters.MilestoneReleaseNotesFilePath, settings);
        }

        if (BuildParameters.ShouldDownloadFullReleaseNotes)
        {
            GitReleaseManagerExport(gitReleaseManagerCredentials.Token, BuildParameters.RepositoryOwner, BuildParameters.RepositoryName, BuildParameters.FullReleaseNotesFilePath, settings);
        }
    })
);
...
BuildParameters.Tasks.PublishReleaseNotesTask = Task("Publish-Release-Notes")
    ...
    .Does(() => RequireTool(BuildParameters.IsDotNetBuild || BuildParameters.PreferDotNetGlobalToolUsage ? ToolSettings.GitReleaseManagerGlobalTool : ToolSettings.GitReleaseManagerTool, () => {
        ...
        if (BuildParameters.RepositoryHostedInGitLab)
        {
            ...
            GitReleaseManagerPublish(gitReleaseManagerCredentials.Token, BuildParameters.RepositoryOwner, BuildParameters.RepositoryName, tagName, publishSettings);
        }
